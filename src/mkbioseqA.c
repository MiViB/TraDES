...
SeqIdPtr LIBCALL MakeRANDId(BiostrucSourcePtr bssp, MoleculeGraphPtr mgp, DbtagPtr dtp)
{
	ValNodePtr vnp;
	SeqIdPtr sip;
	PDBSeqIdPtr psp;
	...
	string_name[0] = '\0';
	StringCpy(string_name,"RND");
	string_name[3] = (Uint1) *chnid;
	string_name[4] = '\0';
	psp->mol = StringSave(string_name);
	psp->chain = (Uint1) *chnid;
	psp->rel = DateCurr();
	return sip;
}
