...
SeqEntryPtr LIBCALL MakeBioseqs(BiostrucPtr bsp, BiostrucResidueGraphSetPtr stdDictionary)
{
  ...
  for (index = 0, currentbp = bp; index < nbp, currentbp != NULL; currentbp = currentbp->next, index++)
  {
	...
	if (currentbp->seq_id->choice == SEQID_PDB)  /* RAND like with no GI */
     {
		 /* this is how a TraDES chain appears, need to copy the fake PDB code chain code ID crafted by ChainCopy.. */
       current_bioseq->id = MakeRANDId(bssp, currentbp, dtp);   
	 }
	else  if (currentbp->seq_id->choice == '\f') /* GI */
	  {
